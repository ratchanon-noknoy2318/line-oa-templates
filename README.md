...
|                Telemedicine Start                |                     More Info                    |                     About Us                     |                    Health Tip                    |
| :----------------------------------------------: | :----------------------------------------------: | :----------------------------------------------: | :----------------------------------------------: |
...
### LINE Chat Loading Indicator
To improve user experience (UX) while fetching hospital data, these templates are designed to work alongside the **LINE Loading API**. 

Before executing heavy database queries or data extraction, trigger the loading indicator via `fetch`:

```javascript
// Trigger LINE Loading Indicator (Valid for 5-60 seconds)
await fetch('https://api.line.me/v2/bot/chat/loading/start', {
  ...
  body: JSON.stringify({
    chatId: event.source.userId, // Target user ID
    loadingSeconds: 5            // Display duration (seconds)
  })
});
```
